...
K_LEN = 6
Q_BLOCK_SIZE = 3 #  M / 4d
KV_BLOCK_SIZE = 3  #  min(M / 4d , d)
Tr = Q_LEN // Q_BLOCK_SIZE
Tc = K_LEN // KV_BLOCK_SIZE

Q = torch.randn(1, 1, Q_LEN, 4, requires_grad=True).to(device='cpu') # [1, 1, Q_LEN, 4]
K = torch.randn(1, 1, K_LEN, 4, requires_grad=True).to(device='cpu') # [1, 1, K_LEN, 4]
V = torch.randn(1, 1, K_LEN, 4, requires_grad=True).to(device='cpu') # [1, 1, K_LEN, 4]
O = torch.zeros_like(Q, requires_grad=True) # [1, 1, Q_LEN, 4]
l = torch.zeros(Q.shape[:-1])[..., None] # [1, 1, Q_LEN, 1]
m = torch.ones(Q.shape[:-1])[..., None] * NEG_INF # [1, 1, Q_LEN, 1]

Q_BLOCKS = torch.split(Q, Q_BLOCK_SIZE, dim=2) #[0] =  [1, 1, Q_BLOCK_SIZE, 4]
K_BLOCKS = torch.split(K, KV_BLOCK_SIZE, dim=2) # [0] = [1, 1, KV_BLOCK_SIZE, 4]
V_BLOCKS = torch.split(V, KV_BLOCK_SIZE, dim=2) #[0] =  [1, 1, KV_BLOCK_SIZE, 4]
O_BLOCKS = list(torch.split(O, Q_BLOCK_SIZE, dim=2))#[0] =  [1, 1, Q_BLOCK_SIZE, 4]
l_BLOCKS = list(torch.split(l, Q_BLOCK_SIZE, dim=2)) #[0] =  [1, 1, Q_BLOCK_SIZE, 1]
m_BLOCKS = list(torch.split(m, Q_BLOCK_SIZE, dim=2)) #[0] =  [1, 1, Q_BLOCK_SIZE, 1]
...
for j in range(Tc):
    ...
    for i in range(Tr):
        ...
        mi = m_BLOCKS[i]
        # 当前这个块的注意力分数
        S_ij = torch.einsum('... i d, ... j d -> ... i j', Qi, Kj) # [1, 1, Q_BLOCK_SIZE, KV_BLOCK_SIZE]
        # 当前这个块的最大值
        m_block_ij, _ = torch.max(S_ij, dim=-1, keepdims=True)
        # 当前这个快的注意力分数P_ij =  e^ {(P_ij - m_block_ij)}
        P_ij = torch.exp(S_ij - m_block_ij)
        # 当这个块 分母 sum(P_ij)
        l_block_ij = torch.sum(P_ij, dim=-1, keepdims=True) + EPSILON # [1, 1, Q_BLOCK_SIZE, 1]
        # 当前这个块的O
        P_ij_Vj = torch.einsum('... i j, ... j d -> ... i d', P_ij, Vj) # [1, 1, Q_BLOCK_SIZE, 4]
        # 当前这个Q组中的最大值
